...
##'
##'
##' @aliases BIKM1_LBM_Binary-class
##'
##' @docType class
##'
##' @keywords class
...
##'
##'
##' @rdname BIKM1_LBM_Binary-class
##'
##' @exportClass BIKM1_LBM_Binary
##'
##' @examples
##'
##' require(bikm1)
...
##'
##' Print method for a \code{\linkS4class{BIKM1_LBM_Binary}} object
##'
##'
##' @param x in the print method, a BIKM1_LBM_Binary object
##'
##' @param ... in the print method, additional parameters (ignored)
##'
##' @export print
##' @aliases print,BIKM1_LBM_Binary-method
##' @examples
...
setMethod("print","BIKM1_LBM_Binary",
          function(x,...){
            result=list(x@criterion_choice,x@criterion_max,x@gopt,x@hopt)
            names(result)=c("criterion_choice","criterion_max","Selected number of row clusters (g)","Selected number of column clusters (h)")
            print(result)
          }
)
...
##'
##'  show method for a \code{\linkS4class{BIKM1_LBM_Binary}} object

##'
##'
##'
##' @param object a BIKM1_LBM_Binary object
##' @export show
##' @aliases show,BIKM1_LBM_Binary-method
##' @examples
##' \donttest{require(bikm1)
##' n=200
##' J=120
...
setMethod("show","BIKM1_LBM_Binary",
          function(object){
            result=list(object@criterion_choice,object@criterion_max,object@gopt,object@hopt)
            names(result)=c("criterion_choice","criterion_max","Selected number of row clusters (g)","Selected number of column clusters (h)")
            print(result)
          }
)
...
##'
##' Produce a summary of informations of a \code{BIKM1_LBM_Binary} object
##'
##'
##'
##' @param object in the summary method, a BIKM1_LBM_Binary object
##' @param ... in the summary method, additional parameters (ignored)
##' @export summary
##'
##' @aliases summary,BIKM1_LBM_Binary-method
##'
##' @examples
##' \donttest{require(bikm1)
##' n=200
...
setMethod("summary","BIKM1_LBM_Binary",
          function(object,...){
            ...
            cat("\nEstimated number of column clusters  (h): ",as.character(object@hopt),"\n",sep="")
            cat("\nEstimated row proportion:",sep="")
            print(object@model_max$theta$pi_g)
            cat("\nEstimated column proportion:",sep="")
            print(object@model_max$theta$rho_h)
            cat("\nEstimated Poisson parameters:\n",sep="")
            print(object@model_max$theta$alpha_gh)
...
##'
##'
##' Plot method for a \code{\linkS4class{BIKM1_LBM_Binary}} object
##'
##'
...
##'
##' @param x an object of class \code{BIKM1_LBM_Binary}.
##'
##' @param y binary matrix of observations.
##'
...
setMethod(
  ...
  definition=function(x,y,...){
    ...
    h=2
    s=2
    pos[1,1]=x@criterion_tab[g,h]
    pos[1,2]=g
    pos[1,3]=h
    if (Niter>1){
    for (iter in 2:Niter){
      if (x@criterion_tab[g+1,h]>-Inf){
        g=g+1
      }else{
        h=h+1
      }
      pos[iter,1]=x@criterion_tab[g,h]
      pos[iter,2]=g
      pos[iter,3]=h
    }
    }
